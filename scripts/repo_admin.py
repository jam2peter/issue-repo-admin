...
COMMAND_RE = re.compile(
    r"/repo-admin create ([A-Za-z0-9._-]{1,100}) (private|public)"
)
...
def load_config(path: str) -> dict[str, Any]:
    ...
    owner_type = str(cfg.get("owner_type", "")).strip().lower()
    if not owner or owner_type not in {"user", "organization"}:
        raise ValueError("owner/owner_type invalid")
    allowed = cfg.get("allowed_author_associations", ["OWNER"])
    if not isinstance(allowed, list) or not all(isinstance(x, str) for x in allowed):
        raise ValueError("allowed_author_associations invalid")
    return cfg
...
def parse_command(body: str) -> CreateCommand:
    ...
    if not match:
        raise ValueError(
            "expected /repo-admin create <name> <private|public>"
        )
...
class GitHubRest:
    def __init__(self, token: str):
        if not token:
            raise ValueError("REPO_ADMIN_TOKEN is not configured")
        self.token = token
    ...
    def request(
        ...
        headers = {
            ...
            "User-Agent": "Issue-Repo-Admin/0.1",
        }
        if data is not None:
...
def provision(
    ...
    owner = str(cfg["owner"]).strip()
    owner_type = str(cfg["owner_type"]).strip().lower()
    ...
    if status == 200:
        ...
        if owner_type == "user":
            ...
            authenticated = str(identity.get("login", "")).strip()
        ...
        payload = {
            "name": command.name,
            "description": str(
                cfg.get("default_description", "Managed by Issue Repo Admin")
            ),
...
def main() -> int:
    ...
    try:
        ...
        result = provision(
            ...
            args.association,
            cmd,
            GitHubRest(os.environ.get("REPO_ADMIN_TOKEN", "")),
        )
